...
export const calculateMonthlyPayment = (loanAmount, months) => {
  ...
  if (safeMonths <= 0) return 0;
  
  // ✅ Rounded to whole rupees
  return Math.round((safeLoanAmount * MURABAHA_TOTAL_MULTIPLIER) / safeMonths);
};
...
export const calculateFIR = (existingObligations, newMonthlyPayment, monthlyIncome) => {
  ...
  if (safeMonthlyIncome <= 0) return 0;

  // ✅ Rounded to 2 decimal places
  return Math.round(
    ((safeExistingObligations + safeNewMonthlyPayment) / safeMonthlyIncome) * 10000
  ) / 100;
};

export const checkEligibility = ({
  monthlyIncome, existingObligations, loanAmount,
  period, employmentType, creditHistory, age, hasGuarantor,
}) => {
  const safeMonthlyIncome = Number(monthlyIncome) || 0;
  const safeExistingObligations = Number(existingObligations) || 0;
  const safeLoanAmount = Number(loanAmount) || 0;
  ...
  // ✅ Fixed cap — max is 100 not 110
  const normalizedScore = Math.max(0, Math.min(100, score));

  let result = "notEligible";
  if (normalizedScore >= 70) result = "eligible";
  else if (normalizedScore >= 45) result = "conditional";

  return { result, firRatio, monthlyPayment, score: normalizedScore, flags };
};